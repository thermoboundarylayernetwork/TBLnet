...
class FixedDirectionLoss(nn.Module):
    """
    Custom direction loss combining mean angle between predicted and true velocities
    and a cosine similarity penalty. Robust for batches with very small velocities.
    """
    def __init__(self):
        super().__init__()
        self.epsilon = 1e-8
...
def direction_loss(u_pred, v_pred, u_true, v_true, min_magnitude=0.01, eps=1e-8):
    """
    Computes directional loss between predicted and true velocities,
    penalizing the cosine distance only in regions with significant true velocity.
    """
    true_mag = torch.sqrt(u_true ** 2 + v_true ** 2)
    mask = (true_mag > 0.005)  # Only consider regions where the true velocity is sufficiently large
...
def value_loss(u_pred, v_pred, u_true, v_true):
    """
    Value loss: Mean squared error between predictions and ground truth,
    with higher weight for grid points with low true velocity.
    """
    true_mag = torch.sqrt(u_true ** 2 + v_true ** 2)
...
def activation_loss(theta, eta):
    """
    Activation loss: encourages the outputs of theta and eta to have sufficient amplitude
    (avoids being suppressed to zero).
    """
    theta_abs = torch.mean(torch.abs(theta))
    eta_abs = torch.mean(torch.abs(eta))

    # Encourage output amplitude exceeding threshold
    loss_theta = F.relu(0.05 - theta_abs) ** 2
    loss_eta = F.relu(0.03 - eta_abs) ** 2

    # Optionally, keep ratio constraint (commented):
